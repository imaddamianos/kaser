...
class AddProductVcPresenter{
    ...
    func addProduct(productName: String, storeName: String, brand: String, price: String, condition: String, description: String, image: String, storeEmail: String, completion: (Bool) -> Void){
//        let UserId = emailID.replace(target: ".", withString: "-")
//        newID = UserId
        APICalls.shared.addProductInfo(productName: productName, storeName: storeName,brand: brand, price: price, condition: condition, description: description, image: image, storeEmail:storeEmail){[weak self] (isSuccess) in
